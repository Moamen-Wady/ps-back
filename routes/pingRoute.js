...
const getReservations = async (num) => {
  const asset = await Ping.findOne({ num });
  if (!asset) throw new Error("Asset not found");
  return asset.Reservations || {};
};

router.put("/ping/y", async (req, res) => {
  try {
    ...
    await Ping.updateOne({ num }, { $set: { Reservations: assetResv } });
...
router.get("/ping/:id", async (req, res) => {
  try {
    const object = await Ping.findOne({ num: req.params.id });
...
router.put("/ping/g", async (req, res) => {
  try {
